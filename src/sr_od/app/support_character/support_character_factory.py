"""支援角色应用工厂"""

from typing import TYPE_CHECKING

from one_dragon.base.operation.application.application_factory import ApplicationFactory
from one_dragon.base.operation.application_run_record import AppRunRecord
from sr_od.app.support_character import support_character_const

if TYPE_CHECKING:
    from one_dragon.base.operation.one_dragon_context import OneDragonContext


class SupportCharacterFactory(ApplicationFactory):

    def __init__(self, ctx: 'OneDragonContext'):
        self.ctx = ctx
        super().__init__(
            app_id=support_character_const.APP_ID,
            app_name=support_character_const.APP_NAME,
            default_group=support_character_const.DEFAULT_GROUP,
            need_notify=support_character_const.NEED_NOTIFY,
        )

    def create_application(self, instance_idx: int, group_id: str):
        from sr_od.app.support_character.support_character_app import SupportCharacterApp
        return SupportCharacterApp(self.ctx)

    def create_run_record(self, instance_idx: int) -> AppRunRecord:
        from sr_od.app.support_character.support_character_run_record import SupportCharacterRunRecord
        from sr_od.context.sr_context import SrContext
        ctx: SrContext = self.ctx
        return SupportCharacterRunRecord(
            instance_idx,
            ctx.game_account_config.game_refresh_hour_offset
        )
