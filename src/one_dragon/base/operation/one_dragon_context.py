...
class ContextInstanceEventEnum(Enum):
    ...
    context_init_done = 'context_init_done'  # 初始化完成，可以运行应用了
    ...
    def init(self) -> None:
        ...
        try:
            ...
            if not self._application_registered:  # 只需要注册一次
                ...
                self.app_group_manager.set_default_apps(self.run_context.default_group_apps)
                self.app_group_manager.clear_config_cache()  # 清除缓存，使配置在下次获取时重新加载
            ...
            self.dispatch_event(ContextInstanceEventEnum.context_init_done.value)  # 通知 UI 初始化完成
